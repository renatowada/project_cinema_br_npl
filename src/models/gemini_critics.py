...
# Limita aspectos e níveis para garantir consistência e evitar erros de formatação
...
# Define o molde de saída para a análise crítica detalhada
...
# Tradutor do Gemini para Python
...
llm = ChatGoogleGenerativeAI(
    model="gemini-2.5-flash-lite", # melhor custo-benefício para tarefas estruturadas, mais rápido e econômico que o Gemini Pro, mas ainda muito capaz para análise de texto e extração de insights
    temperature=0.1, # Não deve ser criativo
...
DIRETRIZES DE EXTRAÇÃO (REGRAS DE OURO):
1. TRADUÇÃO TÉCNICA: Identifique o aspecto técnico real (ex: "filme lento" -> 'Montagem' ou 'Roteiro').
2. CATEGORIAS ESTRITAS: [Roteiro, Direção, Fotografia, Atuação, Montagem, Som, Arte].
3. LIMITE DE CONSENSO: 
   - Liste ATÉ 5 pontos positivos (elogios mais repetidos).
   - Liste ATÉ 5 pontos negativos (críticas mais repetidas).
   - Se houver menos que 5, traga apenas o que existir. Se não houver nenhum, deixe a lista vazia.
4. SÍNTESE: Não extraia frases soltas de usuários. Crie frases que resumam o sentimento da maioria.
"""
...
Converta análises textuais em métricas estruturadas.
...
4. FILTRO FINAL: Garanta que o JSON contenha no máximo 5 itens positivos e 5 itens negativos. Remova redundâncias.
"""
...
# Função para executar a pipeline completa de análise e extração de tags
...
# Função principal para processar as resenhas dos filmes e gerar análises estruturadas via Gemini
def processar_resenhas():
    ...
    for i, filme in enumerate(dados_filmes):
        ...
        if qtd_resenhas <= 3:
            ...
            print("   -> Processando via API Gemini")
            try:
                texto_resenhas = "\n---\n".join(resenhas)
    ...
    print(f"\n Extração via LLM concluída. Salvo em: {output_path}")

if __name__ == "__main__":
    os.makedirs(PROCESSED_DIR, exist_ok=True)
    processar_resenhas()
    print("\n Processamento finalizado. Rode o script build_master_parquet.py para consolidar os dados para análise e visualização.")